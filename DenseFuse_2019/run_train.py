...
if __name__ == "__main__":
    ...
    #           网络模型
    # ----------------------------------------------------#
    model_name = "DenseFuse"  # 模型初始化
    in_channel = 1 if args.gray else 3
    out_channel = 1 if args.gray else 3
    model_train = fuse_model(model_name, input_nc=in_channel, output_nc=out_channel)
    ...
    if True:
        ...
        if args.resume_path is not None:
            ...
            # model_train.encoder.load_state_dict(checkpoint['encoder_state_dict'])
            # model_train.decoder.load_state_dict(checkpoint['decoder_state_dict'])
