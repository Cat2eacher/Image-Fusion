...
def cv_image_points_resize(image, points, size):
    """
    将图像和坐标点都变换为指定size下。
    Args:
        image(nd.array):Original image
        points (list[tuple]): List of points as tuples (x, y) in the original image.
        size (tuple): Resized image size as a tuple (new_width, new_height).
    Returns:
        image(nd.array):resized image
        points (list[tuple]): List of transformed points as tuples (new_x, new_y) in the resized image
    """
    # 放缩尺度
    height, width = image.shape[:2]
    width_scale = size[0] / width
    height_scale = size[1] / height
    # 图像的变换
    resized_image = cv.resize(image, (size[0], size[1]))
    # 坐标的变换
    resized_points = []
    for point in points:
        new_x = round(point[0] * width_scale)
        new_y = round(point[1] * height_scale)
